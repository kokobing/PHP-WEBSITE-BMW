...
//取出信息
$strSQL="SELECT id_newsdir,name,lang,ordernum FROM newsdir WHERE level='1' and dele='1' order by ordernum desc";
$objDB->Execute($strSQL);
$arrnewsdir1=$objDB->GetRows();

$newsdir1='<select name=ajax_newsdir1 id=ajax_newsdir1 >';
for($i=0;$i<sizeof($arrnewsdir1);$i++){
$newsdir1=$newsdir1.'<option value='.$arrnewsdir1[$i][id_newsdir].'>'.$arrnewsdir1[$i][name].'</option>';
}
        ...
        <tr>
          <td height="36" align="left" background="../inc/pics/lanmubg.gif"><table width="200" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td width="153" class="txt_lanmu">信息分类</td>
            </tr>
          </table></td>
        </tr>
        <tr>
		   ...
		   <?php  for($i=0;$i<sizeof($arrnewsdir1);$i++){?>
            <tr>
              <td height="30" align="left" valign="bottom" class="txt_leftmenu"><table width="100%" border="0" cellspacing="5" cellpadding="5">
                  <tr>
                    <td class="txt_lanmu">
					<?php if($onuserperm_editprem=='1'){echo '<span onclick=newsdiredit('.$arrnewsdir1[$i][id_newsdir].',"1",'.$onuserperm_deleprem.','.$arrnewsdir1[$i][ordernum].','.$arrnewsdir1[$i][lang].'); style="cursor:pointer">'.$arrnewsdir1[$i][name].'</span>';} else{ echo $arrnewsdir1[$i][name];}?>
                    </td>
                  </tr>
                  <tr>
                    <td align="center"><table width="85%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
						...
						<?php
						$strSQL="SELECT name,id_newsdir,lang,ordernum FROM newsdir WHERE level='2' and dele='1' and fatherid='".$arrnewsdir1[$i][id_newsdir]."' order by ordernum ASC";
                        $objDB->Execute($strSQL);
                        $arrnewsdir2=$objDB->GetRows();
						for($j=0;$j<sizeof($arrnewsdir2);$j++){if($onuserperm_editprem=='1'){
echo '<span onclick=newsdiredit('.$arrnewsdir2[$j][id_newsdir].',"2",'.$onuserperm_deleprem.','.$arrnewsdir2[$j][ordernum].','.$arrnewsdir2[$j][lang].');  style="cursor:pointer">'.$arrnewsdir2[$j][name].'</span>'."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";}else{echo $arrnewsdir2[$j][name];}
						}
