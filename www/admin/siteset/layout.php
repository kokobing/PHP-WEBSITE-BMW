...
//交换图片位置
	if(isset($_GET["action"]) && $_GET["action"]=="picswitch"){
		$strSQL="UPDATE layoutpic SET ordernum=".$_GET[swtorder]." WHERE id_layoutpic=".$_GET[id]."";
		$objDB->Execute($strSQL);
		$strSQL="UPDATE layoutpic SET ordernum=".$_GET[order]." WHERE id_layoutpic=".$_GET[swtid]."";	
		$objDB->Execute($strSQL);
	    header('Location:layout.php?03'.$_GET[id_layout]);ob_end_flush(); exit();//跳转
	}	
        ...
        <tr>
          <td colspan="2" align="center"><table width="95%" border="0" cellspacing="0" cellpadding="5">
		   ...
		   <?php for($i=0;$i<$intNews;$i++) {?>
            <tr>
              <td width="40%" height="30" align="left" valign="middle" class="txt_leftmenu"><a href="layout.php?03<?php echo $arrnewsinfos[$i][id_layout];?>"  class="txt_leftmenu"><?php echo cutstr($arrnewsinfos[$i][title],60,1);?></a></td>
              <td width="52%" align="left" valign="middle" class="txt_leftmenu"><a href="layout.php?03<?php echo $arrnewsinfos[$i][id_layout];?>"  class="txt_leftmenu"><?php //echo cutstr($arrnewsinfos[$i][intro],160,1);?></a></td>
...
<div id="lineout">
          ...
          <td align="center"><table width="93%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <table width="100%" border="0" cellpadding="4" cellspacing="0" class="txt">
                  ...
                  <tr bgcolor="#FFFFFF">
                    <td valign="top" bgcolor="#FFFFFF">信息</td>
   ...
   $strSQL="SELECT opicname,id_layoutpic,type,title,ordernum FROM layoutpic WHERE id_layout=$onenewsid order by ordernum asc";
   $objDB->Execute($strSQL);
          ...
          <td align="center"><table width="93%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <table width="100%" border="0" cellpadding="4" cellspacing="0" class="txt">
                  <tr bgcolor="#FFFFFF">
                    <td width="91" valign="top" bgcolor="#FFFFFF">标题</td>
                    <td bgcolor="#FFFFFF"><input name="title" type="text" class="input_01" id="newstitle" style="width:500px" value="<?php echo $onenewsinfo[title];?>"  readonly="readonly" /></td>
                    </tr>
                                      <tr>
                                        <td colspan="3" align="left" valign="middle" bgcolor="#EBF3F9"  style="padding:0; height:1px;"></td>
                        </tr>
...
<span class="txt_underpic" onclick="delpic(<?php echo $onenewspicinfo[$i][id_layoutpic];?>);" style="cursor:pointer; color:#FFF">删除</span>

<?
$picnum=sizeof($onenewspicinfo);
if($i==0){
	$pic_swtid=$onenewspicinfo[$picnum-1][id_layoutpic];
	$pic_swtorder=$onenewspicinfo[$picnum-1][ordernum];
}else{
	$pic_swtid=$onenewspicinfo[$i-1][id_layoutpic];
	$pic_swtorder=$onenewspicinfo[$i-1][ordernum];
}

if($i==($picnum-1)){
	$pic_swtid2=$onenewspicinfo[0][id_layoutpic];
	$pic_swtorder2=$onenewspicinfo[0][ordernum];
}else{
	$pic_swtid2=$onenewspicinfo[$i+1][id_layoutpic];
	$pic_swtorder2=$onenewspicinfo[$i+1][ordernum];
}

?>
  <a href='layout.php?action=picswitch&id=<?=$onenewspicinfo[$i][id_layoutpic];?>&order=<?=$onenewspicinfo[$i][ordernum];?>&swtid=<?=$pic_swtid;?>&swtorder=<?=$pic_swtorder;?>&id_layout=<?=$onenewsinfo[id_layout];?>'>
  <img src="../inc/pics/icon_006.gif" width="10" height="10" border="0" style="position:absolute;bottom:7px;left:0px;" />
  </a>
  
  <a href='layout.php?action=picswitch&id=<?=$onenewspicinfo[$i][id_layoutpic];?>&order=<?=$onenewspicinfo[$i][ordernum];?>&swtid=<?=$pic_swtid2;?>&swtorder=<?=$pic_swtorder2;?>&id_layout=<?=$onenewsinfo[id_layout];?>'>
